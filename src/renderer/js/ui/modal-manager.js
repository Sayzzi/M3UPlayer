...
M3U.ModalManager = class {
  ...
  showAddPlaylist() {
    ...
    const modal = M3U.dom.el('div', { className: 'modal' }, [
      ...
      M3U.dom.el('div', { className: 'modal-footer' }, [
        M3U.dom.el('button', {
          className: 'btn', textContent: 'Cancel',
          onClick: () => this.close()
        }),
        M3U.dom.el('button', {
          className: 'btn btn-accent', textContent: 'Add & Load',
          onClick: () => {
            if (activeTab === 'm3u') {
  ...
  async _addXtream(server, username, password, name) {
    ...
    try {
      ...
      const entry = await this.playlistService.addPlaylist(
        server, name, 'xtream', { server, username, password }
      );
      await this.playlistService.setActive(entry.id);
  ...
  async showManagePlaylists() {
    ...
    for (const pl of playlists) {
      ...
      const subtitle = pl.type === 'xtream' && pl.xtream
      ...
      const item = M3U.dom.el('div', {
        ...
        M3U.dom.el('div', { className: 'playlist-item-actions' }, [
          M3U.dom.el('button', {
            className: 'btn', textContent: 'Load',
            onClick: async () => {
              try {
                this.close();
                M3U.dom.dispatch('loading-start', { text: 'Loading playlist...' });
                ...
                if (pl.type === 'xtream' && pl.xtream) {
                  await this.playlistService.loadFromXtream(
                    pl.xtream.server, pl.xtream.username, pl.xtream.password
                  );
                } else {
  ...
  showSeriesEpisodes(series, info, creds) {
    ...
    for (const seasonNum of seasons) {
      ...
      listEl.appendChild(M3U.dom.el('div', {
        ...
        const item = M3U.dom.el('div', {
          ...
          M3U.dom.el('div', { className: 'playlist-item-info' }, [
            ...
            ep.info && ep.info.duration
              ? M3U.dom.el('div', { className: 'playlist-item-url', textContent: ep.info.duration })
              : null
          ].filter(Boolean))
        ]);
        listEl.appendChild(item);
